...
class DDQNAgent:
    ...
    def __init__(self, data, gamma=0.95, num_neutron=24, epsilon_min = 0.001, epsilon_decay=0.995, 
                 recent_k = 0, capital=60000.0, init_order_ratio=0.7,
                 external_states = ["current_price", "rolling_mean", "rolling_std", 
                                 "cross_upper_band", "cross_lower_band"],
                 internal_states = ["stock", "cash", "total_value"], verbose=False):
        ...
        self.env = Environment(data=data, states=external_states, recent_k=recent_k)
        
        # Internal states
        self.internal_states = internal_states
        self.portfolio = Portfolio(final_price=self.env.getFinalPrice(), capital=capital, 
                                   init_order_ratio=init_order_ratio, verbose=verbose, states=internal_states)
        
        # NN model
        _state_size = len(external_states) + len(internal_states)
        self.model = QValue_NN(_state_size, len(list(Action)), num_neutron)
        self.target_model = QValue_NN(_state_size, len(list(Action)), num_neutron)
    ...
    def train(self, num_episodes=100):
        ...
        for i in range(num_episodes):
            ...
            state = self.env.getCurrentStates() + self.portfolio.getCurrentStates()
            ...
            while (True):
                ...
                next_state = next_state + self.portfolio.getCurrentStates()
